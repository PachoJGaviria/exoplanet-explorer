...
(function (document) {
  ...
  function get (url) {
    return new Promise((resolve, reject) => {
      /* This code is the action in the promise! */
      var req = new XMLHttpRequest()
      req.open('GET', url)
      req.onload = () => {
        if (req.status === 200) {
          // It worked! - You'll want to resolve with the data from req.response
          resolve(req.response)
        } else {
          // It failed - Be nice and reject with req.statusText
  ...
  window.addEventListener('WebComponentsReady', function () {
    ...
    You'll need to add a .then and a .catch. Pass the response to addSearchHeader on resolve
    or pass 'unknown' to addSearchHeader if it rejects.
     */
